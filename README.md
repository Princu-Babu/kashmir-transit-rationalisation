...
Built for the Principal Secretary of Transport, J&K — this engine ingests 613 registered route permits (minibuses, e-buses, MPS buses, JKRTC city/regional services), clips them to the **Srinagar Valley study area** (33.5°–34.5° N, 74.4°–75.2° E), and produces a rationalised frequency plan for **342 routes** = **312 permit-derived survivors + 30 synthetic SSCL/CHALO e-bus trunk injections** (the 30 are hardcoded in the engine, not rows in `existing-routes.csv`). Results are overlaid against WorldPop population rasters and OpenStreetMap Points of Interest to generate fleet allocation, headway schedules, and interactive maps.

> **⚠ Accuracy caveat (independent audit, June 2026).** The reduction from 613 permits to 312 survivors is **currently dominated by endpoint-geocoding failures, not a clean bounding-box scoping decision** — only ~11 routes drop for genuine out-of-bbox reasons; ~290 dropped because 118 valley place names (Anantnag, Pampore, Budgam, Pulwama, Sopore, Kulgam, Shopian, …) collapsed onto a single Srinagar coordinate, producing zero-length routes. Endpoint geocoding is being hardened (district-aware, centroid-collision rejection) and the network re-run as **v3.3.8** before any external submission. Remediation status: see [`AUDIT_FIX_LOG.md`](AUDIT_FIX_LOG.md).
