...
| `fix_geometries_v345geo*.py` | **v3.4.5-geo (2026-07-03): redraw the 18 stale map lines left by the v3.4.4 km-only corrections.** Step-1 finding: every stale line WAS the OSRM shortest path from the drawn pins — the endpoint coords were the defect. Fix: re-anchor termini (stops master / observed GPS stops / researched pins — JVC=SKIMS Bemina 34.0839/74.7614; Iskanderpora+Badran are Beerwah-belt Budgam villages, NOT Baramulla) → OSRM re-route → accept only within the deep-dive ledger's cited km band (+15% one-way grace); prefer observed GPS runs where ≥2 exist. **15/18 FIXED, 3 honest holdouts (FDR-269/108/406 — one-way systems push OSRM past band; flagged in `geometry_fixes_v345geo.csv`)**. Geometry-only: CSV/fleet byte-identical (fleet resum 1,011 ✓). Propagated: dashboard geojson + plan_evidence + evidence.json + reconciliation + 15 regenerated route-map HTMLs. Side-effect: route-evidence STRONG 172→171 (one redrawn line honestly less fragment-covered than the old wrong line). |
