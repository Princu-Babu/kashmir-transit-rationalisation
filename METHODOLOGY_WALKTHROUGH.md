...
All numbers below were re-verified against the live v3.3.8 outputs on 2026-06-20.
...
**Verified:** `existing-routes.csv` = **401 routes, 0 centroid-collapse, 0
...
**Verdict: ✅ this is the fix that makes the whole plan legitimate.** Residue: ~21
minor localities still need manual coding (transparent, in the reject file).

## STAGE 2 — Ingestion + SSCL backbone injection
401 geocoded routes − 11 outside the study box + **30 hardcoded SSCL/CHALO trunks**
= **420 routes**. SSCL fleet (98 buses) matches the SSCL sheet's "New Deployment"
...
length. Fixed a bug that silently discarded via-waypoints (now 257/401 routed via
their waypoints).

**Verified:** lengths physically sane — median 14.4 km, longest Srinagar→Qazigund
...
so "95.7%" was inflated ~3×. **Honest figure: 1,588,964 residents within 400 m =
31.1% of the 5.1 M study-area population.**

**Verdict: ✅ 31% is defensible** (≈ the full Srinagar urban core + district reach;
